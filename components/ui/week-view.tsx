  ...
  const processedDayEvents = useMemo(() => {
    const result = days.map((day) => {
      ...
      const dayEvents = events.filter((event) => {
        ...
        if (event.allDay || isMultiDayEvent(event as unknown as TCalendarEvent))
          return false;
        const eventStart = new Date(event.start);
        const eventEnd = new Date(event.end);
        ...
        <div className="border-border/70 bg-muted/50 border-b">
          <div className="grid grid-cols-8">
            <div className="border-border/70 relative border-r">
              <span className="text-muted-foreground/70 absolute bottom-0 left-0 h-4 w-16 max-w-full pe-2 text-right text-[10px] sm:pe-4 sm:text-xs">
                O dia todo
              </span>
            </div>
      ...
      <div className="grid flex-1 grid-cols-8 overflow-hidden">
        <div className="border-border/70 border-r grid auto-cols-fr">
          {hours.map((hour, index) => (
            ...
            >
              {index > 0 && (
                <span className="bg-background text-muted-foreground/70 absolute -top-3 left-0 flex h-5 w-18 max-w-full items-center justify-end pe-2 text-[10px] sm:pe-4 sm:text-xs">
