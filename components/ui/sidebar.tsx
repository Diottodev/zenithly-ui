...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (nextValue: boolean | ((prevOpen: boolean) => boolean)) => {
      const openState =
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      {open ? (
        <RiSkipLeftLine aria-hidden="true" className="size-5.5" size={22} />
      ) : (
        <RiSkipRightLine aria-hidden="true" className="size-5.5" size={22} />
      )}
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      className={cn(
        'mb-3 flex shrink-0 items-center rounded-md px-2 font-medium text-sidebar-foreground/70 text-xs outline-hidden ring-sidebar-ring transition-[margin,opacity] duration-200 ease-in-out focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
